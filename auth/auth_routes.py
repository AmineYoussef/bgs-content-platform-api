...
def register() -> ListSuccessResponse:
    try:
        ...
        username_exists = auth_controller.verify_duplicate_username(user_input.username)
        if username_exists:
            raise ValueError("Username already taken")
        email_exists = auth_controller.verify_duplicate_email(user_input.email)
        if email_exists:
            raise ValueError("Email already taken")

        user = auth_controller.register_user(user_input)
        del user.password
...
def login():
    try:
        ...
        user = auth_controller.login(
            user_crendentials.email,
            user_crendentials.password
        )
        # token lasting 24 hrs
